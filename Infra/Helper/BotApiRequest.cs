...
{
    public class BotApiRequest<T>
    {
        //private static String API_URL = "http://localhost:37792/";
        private static String API_URL = "http://altbotapi.azurewebsites.net/";
        //Cloud Token
                    ...
                    using (var client = new HttpClient(new ClientCompressionHandler(new GZipCompressor(), new DeflateCompressor())))
                    {
                        ...
                        using (var response = await client.GetAsync(url))
                        {
                            if (response.IsSuccessStatusCode)
                            {
                                ...
                                var jsonContent = JsonConvert.DeserializeObject<T>(objsJsonString, new JsonSerializerSettings()
                                {
